-- Mayunie DEX Explorer - V3 Delta 720x1280 friendly
-- Teclas: RightCtrl alterna visibilidade
if not game:IsLoaded() then game.Loaded:Wait() end

-- Ambiente mínimo compatível
...
local COMMON_PROPS = {
    ...
    ["TextBox"]={"Text","TextSize","TextColor3","TextTransparency","ClearTextOnFocus","MultiLine"},
...
-- Tamanho pensado para 720x1280 portrait
local function calcWindow()
    ...
    local padX, padY = 24, 80
    local maxW = math.max(480, vp.X - padX)   -- deixa respiro nas bordas
    local maxH = math.max(520, vp.Y - padY)
    local W = math.min(680, maxW)             -- limite superior
    local H = math.min(560, maxH)
    return W, H, vp
end
...
Title.TextXAlignment=Enum.TextXAlignment.Left; Title.Text="Mayunie DEX"; Title.Size=UDim2.new(0,140,1,0); Title.Position=UDim2.new(0,16,0,0); Title.Parent=TopBar

local TopRight = Instance.new("Frame"); TopRight.BackgroundTransparency=1; TopRight.Size=UDim2.new(1,-170,1,0); TopRight.Position=UDim2.new(0,160,0,0); TopRight.Parent=TopBar
local HR = Instance.new("UIListLayout", TopRight); HR.FillDirection=Enum.FillDirection.Horizontal; HR.HorizontalAlignment=Enum.HorizontalAlignment.Right; HR.VerticalAlignment=Enum.VerticalAlignment.Center; HR.Padding=UDim.new(0,6)
...
SearchBox.Size=UDim2.new(0,200,0,28); SearchBox.BackgroundColor3=Theme.bg; SearchBox.TextColor3=Theme.text
SearchBox.PlaceholderColor3=Theme.subtext; SearchBox.PlaceholderText="Buscar Nome ou Classe"
...
local BtnEdit = pill(96, "Editar: OFF")
local BtnRefresh = pill(84, "Refresh")
local BtnCopy = pill(100, "Copy Path")
...
local LeftPane = Instance.new("Frame"); LeftPane.BackgroundColor3=Theme.panel; LeftPane.Size=UDim2.new(0,320,1,0); LeftPane.Name="LeftPane"; LeftPane.Parent=Body; Instance.new("UIStroke",LeftPane).Color=Theme.stroke
local Splitter = Instance.new("Frame"); Splitter.BackgroundColor3=Theme.stroke; Splitter.Size=UDim2.new(0,4,1,0); Splitter.Position=UDim2.new(0,320,0,0); Splitter.Name="Splitter"; Splitter.Parent=Body
local RightPane = Instance.new("Frame"); RightPane.BackgroundColor3=Theme.panel; RightPane.Position=UDim2.new(0,324,0,0); RightPane.Size=UDim2.new(1,-324,1,0); RightPane.Name="RightPane"; RightPane.Parent=Body; Instance.new("UIStroke",RightPane).Color=Theme.stroke

-- Tree scroll com auto height
...
local PropLayout = Instance.new("UIListLayout"); PropLayout.Padding=UDim2.new(0,4); PropLayout.SortOrder=Enum.SortOrder.LayoutOrder; PropLayout.Parent=PropScroll
...
local function rowProperty(name, value, inst, allowEdit)
    ...
    L.Text=name; L.Size=UDim2.new(0.38,-8,1,0); L.Position=UDim2.new(0,8,0,0); L.Parent=Row
    ...
    V.Text=serialize(value); V.Size=UDim2.new(0.62,-12,1,-8); V.Position=UDim2.new(0.38,4,0,4); Instance.new("UICorner",V).CornerRadius=UDim.new(0,6); if allowEdit then Instance.new("UIStroke",V).Color=Theme.stroke end
    V.Parent=Row
    if allowEdit then
...
local function buildPropertyPanel(inst, allowEdit)
    ...
    if attrs and next(attrs)~=nil then
        ...
        for k,v in pairs(attrs) do
            ...
            Instance.new("UICorner",Row).CornerRadius=UDim.new(0,6); Instance.new("UIStroke",Row).Color=Theme.stroke
            local L=Instance.new("TextLabel"); L.BackgroundTransparency=1; L.Font=Enum.Font.GothamSemibold; L.TextSize=13; L.TextColor3=Theme.text; L.TextXAlignment=Enum.TextXAlignment.Left
            L.Text="[Attr] "..k; L.Size=UDim2.new(0.38,-8,1,0); L.Position=UDim2.new(0,8,0,0); L.Parent=Row
            ...
            V.Text=serialize(v); V.Size=UDim2.new(0.62,-12,1,-8); V.Position=UDim2.new(0.38,4,0,4); Instance.new("UICorner",V).CornerRadius=UDim.new(0,6); Instance.new("UIStroke",V).Color=Theme.stroke; V.Parent=Row
...
ResultsDrop.Size=UDim2.fromOffset(200,180); ResultsDrop.Parent=Main
...
local function runSearch(q)
    ...
    local function scan(root)
        for _,d in ipairs(root:GetDescendants()) do
            ...
            if ok1 and ok2 and (nm:lower():find(q,1,true) or cn:lower():find(q,1,true)) then table.insert(matches,d); if #matches>=150 then return end end
        end
    end
    for _,root in ipairs(ROOTS) do pcall(scan,root); if #matches>=150 then break end end
    ...
    ResultsDrop.Size = UDim2.fromOffset(sz.X, 180)
...
do
    ...
    UIS.InputChanged:Connect(function(input)
        if resizing and input.UserInputType==Enum.UserInputType.MouseMovement then
            local delta=input.Position-dragStart; local w=math.clamp(startX+delta.X,220,520)
...
local function buildTreeSafe()
    local ok, err = pcall(buildTree)
    if not ok then setStatus("Erro ao montar árvore: "..tostring(err)) end
end
